...
@Config
@Autonomous
public class AutoRedNTXTesting extends LinearOpMode {
...
//    public static double L_ROTATE_DEPOSIT = GlideConstants.L_ROTATE_DEPOSIT;
    ...
    LinearInterpolator linInterp2 = new LinearInterpolator(Math.toRadians(-90), Math.toRadians(-90));
    LinearInterpolator linInterp5 = new LinearInterpolator(Math.toRadians(0), Math.toRadians(-5));

    private static double stoneOffset = 5;
...
//        rin.setDirection(DcMotor.Direction.FORWARD);
        ...
        while (!opModeIsActive() && !isStopRequested()) {
            telemetry.addData("RED AUTO", true);
...
//        LsetArm(L_ARM_STOW);
        ...
        followTrajectoryArmSync(
                drive.trajectoryBuilderFast()
                        .lineTo(new Vector2d(STONES_X[STONE_OPTIONS[stonePos][stoneIndex = 0]] + 2, -38), constInterp)
                        .build()
                , State.DEFAULT
        );
        followTrajectoryArmSync(
                drive.trajectoryBuilderFast()
                        .lineTo(new Vector2d(STONES_X[STONE_OPTIONS[stonePos][stoneIndex = 0]], -35), constInterp)
                        .build()
                , State.DEFAULT
        );
...
//        grab();
        ...
        followTrajectoryArmSync(
                drive.trajectoryBuilder()
                        ...
                        .lineTo(new Vector2d(47, -42), linInterp)
//                        .lineTo(new Vector2d(47,-26), constInterp90)
                        .build()
...
//        }
        ...
        // Move Foundation
        followTrajectoryArmSync(
                drive.trajectoryBuilderFoundation()
        ...
        followTrajectoryArmSync(
                drive.trajectoryBuilder()
//                        .splineTo(new Pose2d(24, -46, Math.toRadians((-180))))
                .lineTo(new Vector2d(24, -46), constInterp180)
                .build()
                , State.DEFAULT
        );
        deposit();
...
//        delay(.25);
        ...
        followTrajectoryArmSync(
                drive.trajectoryBuilder()
                        .lineTo(new Vector2d(12,ALLEY_Y), constInterp180)
                        .lineTo(new Vector2d(STONES_X[STONE_OPTIONS[stonePos][stoneIndex = 2]],ALLEY_Y),constInterp180)
                        .build()
                , State.TO_QUARRY);
        drive.update();
...
//        delay(.25);
        ...
        followTrajectoryArmSync(
                drive.trajectoryBuilder()
                        .lineTo(new Vector2d(12,ALLEY_Y), constInterp180)
                        .lineTo(new Vector2d(STONES_X[STONE_OPTIONS[stonePos][stoneIndex = 3]],ALLEY_Y),constInterp180)
                        .build()
                , State.TO_QUARRY);
        drive.update();
        double stone4 = drive.getPoseEstimate().getY();
        strafeAndGrab(drive, -stone4-31);
...
//            LsetArm(L_ARM_GRAB);
    ...
    public void followTrajectoryArmSync(Trajectory t, State s) {
        ...
        while (!Thread.currentThread().isInterrupted() && drive.isBusy()) {
            TelemetryPacket packet = new TelemetryPacket();
            double imu = 0, odo = 0;
            packet.put("imu heading", imu = Math.toDegrees(drive.getExternalHeading()));
            packet.put("odo heading", odo = Math.toDegrees(drive.getPoseEstimate().getHeading()));
            packet.put("heading diff", (imu - odo));
//            dashboard.sendTelemetryPacket(packet);
            drive.update();
            flipIntakeUpdate();
            switch (s) {
                case TO_FOUNDATION:
                    ...
                    break;
                case TO_QUARRY:
                    if (!updated && drive.getPoseEstimate().getX() < STONE_OPTIONS[stonePos][stoneIndex] + 5) {
                        updated = true;
